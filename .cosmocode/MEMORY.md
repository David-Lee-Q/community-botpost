...
- Instructions:
  - 每批发布完成后 fetch_detail_urls.py 自动抓取：优先从「我的文章」页（usercenter/1447/article/全部文章）点击取真实 URL，以发布接口返回的 article_id 构造 `https://openlab.cosmoplat.com/article-detils?id={id}&articleType=0` 兜底（列表页存在同名/重复文章，避免误记）
...
- Instructions:
  ...
  - 去重：跨源标题去重（归一化标题仅采1次）、URL/标题 hash 去重、24 小时内不重复、多源转载优先权威源；github.com 网页直连超时时用 api.github.com
...
- Instructions:
  ...
  - 最新周期 2026-08-31~2026-09-06 综合分 82，当前短板维度：可读表达、传播表现
  - 改进要点：打磨标题信息量与钩子、摘要直击痛点、首段前三句抓住读者、每段聚焦单一论点并控制段落长度、增强小标题引导与图文呼应
...
- Instructions:
  ...
  - 配图主题化池：IMAGE_POOL 73 张全部为记忆确认主题的 Unsplash 图，按 theme 分组（ai 11/code 9/dc 12/ind 15/data 9/abs 17）；THEMES 映射文章分类→主题组，pick_images(category) 优先分类主题组取图、不足时扩展相邻组、全池耗尽重置（保留 recent 最近9张避免立即重复）；每次发文取3张（正文2+封面1），全局去重记录于 bot/used_images.json；Pexels 随机扫描图（主题未知）已移除
...
- Instructions:
  - 每日 20:05 推送每日总结，每周一 10:00 推送每周总结（runner.py 定时触发）
...
- Category: 运维部署
- Instructions:
  - 每天定时发布 4 篇（DAILY_COUNT=4，8:00~20:00 窗口随机时段）；runner.py 每天 6 点调 gen_plan.py 补足未来 3 天计划（LLM 生成标题/摘要/正文，复用主题图池配图+封面，封面 404 自动换图，单篇失败跳过该时段），到点自动发布并登记台账
...
- Instructions:
  ...
  - 当日补发：bot/backfill_today.py 把当天剩余时段（now+45min 起均匀分布到 19:30）补足 4 篇 pending，runner 到点自动发布；gen_plan.py 只补未来天数不含当天；某选题连续生成失败会跳过该时段致当天不足 4 篇，需次日 gen_plan 或 backfill_today 补齐
